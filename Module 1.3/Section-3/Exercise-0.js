    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].school.toLowerCase() === "east") {
            if (typeof callback === "function") {
              // not working because of the parentesis after callback(...)
              // setTimeout(callback(data[i]), 3000); 
...
// This is working because, we have to make sure that not everything needs to be asynchronous
