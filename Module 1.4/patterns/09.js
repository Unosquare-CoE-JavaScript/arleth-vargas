...
const callFnsInSequense = (...fns) => (...args) => {
    fns.forEach(fn => fn && fn(...args))
}

// This is going to have the initial stae and reset the state to its initial 
const useClapState = (initialState = INITIAL_STATE) => {
    const MAXIMUM_USER_CLAP = 50

    // this is added in order to make this value a const and won't change
    ...
    // glorified counter this will increase the counter everytime reset is called
    const resetRef = useRef(0)

    const prevCount = usePrevious(count)
    // this is going to set state to its inital
    ...
    const getToggleProps = ({ onClick, ...otherProps } = {}) => ({
        onClick: callFnsInSequense(updateClapState, onClick),
...
const Usage = () => {
    const { clapState, 
        ...
        getCounterProps, 
        reset, 
        resetDep } = useClapState(userInitialState)
